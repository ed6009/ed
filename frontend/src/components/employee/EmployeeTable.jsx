...
const EditEmployeeModal = ({ employee, patchEmployee }) => {
  const [emp_name, setEmp_name] = useState(employee.emp_name);
  const [status, setStatus] = useState(employee.status);
  const [qualification, setQualification] = useState(employee.qualification);

  const handleSubmit = (e) => {
    e.preventDefault();
    patchEmployee(employee.emp_id, {
      emp_name,
      status,
      qualification,
    });
  ...
  return (
    <dialog id={`my_modal_${employee.emp_id}`} className="modal">
...
const EmployeeTable = () => {
    ...
    () => [
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <div>
            <RoleAssignModal
              emp_id={row.original.emp_id}
              emp_name={row.original.emp_name}
            />
          </div>
        ),
      },
        ...
        Cell: ({ row }) => (
          <div className="flex items-center justify-center">
            <button
              ...
              onClick={() =>
                document
                  .getElementById(`my_modal_${row.original.emp_id}`)
                  .showModal()
              }
            >
              <svg
            ...
            <EmployeeProfileModal
              ...
              fetchEmpData={row.original.fetchEmpData}
            />
          </div>
        ),
      },
  ...
  const patchEmployee = useCallback((emp_id, updatedData) => {
    ...
    <div>
      ...
      <div className="mx-8 border border-gray-300 rounded-lg table-container">
        <div className="flex items-center justify-between pb-2 mx-20 mt-2 shadow-sm">
          ...
          <AddEmployeeModal fetchEmpData={fetchEmpData} />
        </div>
        <div className="flex items-center justify-between pb-2 mx-20 mt-2 shadow-sm">
