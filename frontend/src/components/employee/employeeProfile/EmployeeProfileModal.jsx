...
const EmployeeProfileModal = (props) => {
  ...
  useEffect(() => {
    getSingleEmployeeProfileData(props.emp_id);
  }, [props.emp_id]);

  function getSingleEmployeeProfileData(emp_id) {
    fetch(`http://localhost:8080/getsingleemppro/${emp_id}`)
      .then((res) => res.json())
      .then((result) => {
        setProfile(result);
        if (result.length === 0) {
  ...
  const handleShowProfile = () => {
    if (profile.length === 0) {
      document.getElementById(`comp_${props.emp_id}`).showModal();
    } else {
      document.getElementById(`modal_${props.emp_id}`).showModal();
    }
  };

    const postEmpProfile = (e) => {
      ...
      fetch("http://localhost:8080/postemppro", {
        method: "POST",
        body: data,
      })
  ...
  return (
    <>
      ...
      <dialog id={`comp_${props.emp_id}`} className="modal">
        <div className="modal-box">
          <h3 className="text-lg font-bold">Please Complete Your Profile</h3>
          <form onSubmit={postEmpProfile}>
      ...
      {profile.map((currEle, index) => {
        ...
        return (
          <div key={index}>
            <dialog id={`modal_${props.emp_id}`} className="modal">
              ...
              <form method="dialog" className="modal-backdrop">
                <button
                  onClick={() =>
                    document.getElementById(`modal_${props.emp_id}`).close()
                  }
                >
                  Close
                </button>
